...
//
	 ...
	 *
	 */
	class Yourseobook_Upsell_Section extends Yourseobook_Custom_Section {
		/**
		 * The type of control being rendered
		 */
		public $type = 'yourseobook-upsell';
		/**
		 * The Upsell URL
		 */
		public $url = '';
		...
		protected function render() {
			...
			?>
			<li id="accordion-section-<?php echo esc_attr( $this->id ); ?>" class="yourseobook_upsell_section accordion-section control-section control-section-<?php echo esc_attr( $this->id ); ?> cannot-expand">
	 ...
	 */
	if ( ! function_exists( 'yourseobook_url_sanitization' ) ) {
		function yourseobook_url_sanitization( $input ) {
	 ...
	 */
	if ( ! function_exists( 'yourseobook_switch_sanitization' ) ) {
		function yourseobook_switch_sanitization( $input ) {
			if ( true === $input ) {
				return 1;
			} else {
				return 0;
	 ...
	 * @return integer	Sanitized value
	 */
	if ( ! function_exists( 'yourseobook_radio_sanitization' ) ) {
		function yourseobook_radio_sanitization( $input, $setting ) {
	 ...
	 */
	if ( ! function_exists( 'yourseobook_sanitize_integer' ) ) {
		function yourseobook_sanitize_integer( $input ) {
			return (int) $input;
		}
	}
	 ...
	 * @return string	Sanitized input
	 */
	if ( ! function_exists( 'yourseobook_text_sanitization' ) ) {
		function yourseobook_text_sanitization( $input ) {
			if ( strpos( $input, ',' ) !== false) {
				$input = explode( ',', $input );
			}
			if( is_array( $input ) ) {
	 ...
	 */
	if ( ! function_exists( 'yourseobook_array_sanitization' ) ) {
		function yourseobook_array_sanitization( $input ) {
			if( is_array( $input ) ) {
				foreach ( $input as $key => $value ) {
					$input[$key] = sanitize_text_field( $value );
				}
	 ...
	 * @return string	Sanitized input
	 */
	if ( ! function_exists( 'yourseobook_hex_rgba_sanitization' ) ) {
		function yourseobook_hex_rgba_sanitization( $input, $setting ) {
			...
			if ( false === strpos( $input, 'rgba' ) ) {
				...
				$input = 'rgba(' . yourseobook_in_range( $red, 0, 255 ) . ',' . yourseobook_in_range( $green, 0, 255 ) . ',' . yourseobook_in_range( $blue, 0, 255 ) . ',' . yourseobook_in_range( $alpha, 0, 1 ) . ')';
			}
			return $input;
		}
	}
	 ...
	 */
	if ( ! function_exists( 'yourseobook_in_range' ) ) {
		function yourseobook_in_range( $input, $min, $max ){
			if ( $input < $min ) {
				$input = $min;
			}
			if ( $input > $max ) {
	 ...
	 * @return string	Sanitized input
	 */
	if ( ! function_exists( 'yourseobook_google_font_sanitization' ) ) {
		function yourseobook_google_font_sanitization( $input ) {
	 ...
	 * @return string	Sanitized input
	 */
	if ( ! function_exists( 'yourseobook_date_time_sanitization' ) ) {
		function yourseobook_date_time_sanitization( $input, $setting ) {
	 ...
	 * @return string	Sanitized input
	 */
	if ( ! function_exists( 'yourseobook_range_sanitization' ) ) {
		function yourseobook_range_sanitization( $input, $setting ) {
			...
			return yourseobook_in_range( $number, $min, $max );
		}
	}
