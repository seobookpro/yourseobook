jQuery( document ).ready(function($) {
		...
		if(numRepeaterItems > 0) {
			...
			if(numRepeaterItems > 1) {
				...
				for (i = 1; i < numRepeaterItems; ++i) {
					yourseobookAppendRow($(this), defaultValuesArray[i]);
				}
			}
		}
	});
	...
	$(this).find('.sortable_repeater.sortable').sortable({
		update: function(event, ui) {
			yourseobookGetAllInputs($(this).parent());
		}
	});
	...
	$('.sortable_repeater.sortable').on('click', '.customize-control-sortable-repeater-delete', function(event) {
		...
		if(numItems > 1) {
			$(this).parent().slideUp('fast', function() {
				...
				$(this).remove();
				yourseobookGetAllInputs(parentContainer);
			})
		}
		else {
			$(this).parent().find('.repeater-input').val('');
			yourseobookGetAllInputs($(this).parent().parent().parent());
		}
	});
	...
	$('.customize-control-sortable-repeater-add').click(function(event) {
		event.preventDefault();
		yourseobookAppendRow($(this).parent());
		yourseobookGetAllInputs($(this).parent());
	});
	...
	function yourseobookAppendRow($element, defaultValue = '') {
	...
	function yourseobookGetAllInputs($element) {
	...
	$('.multi-image-checkbox').on('change', function () {
	  yourseobookGetAllImageCheckboxes($(this).parent().parent());
	});

	// Get the values from the checkboxes and add to our hidden field
	function yourseobookGetAllImageCheckboxes($element) {
	...
	$( ".pill_checkbox_control .sortable" ).sortable({
		...
		update: function( event, ui ) {
			yourseobookGetAllPillCheckboxes($(this).parent());
		}
	});

	$('.pill_checkbox_control .sortable-pill-checkbox').on('change', function () {
		yourseobookGetAllPillCheckboxes($(this).parent().parent().parent());
	});

	// Get the values from the checkboxes and add to our hidden field
	function yourseobookGetAllPillCheckboxes($element) {
	...
	$('.google-fonts-list').on('change', function() {
		...
		var indexes = $.map(bodyfontcontrol.yourseobookfontslist, function(obj, index) {
		...
		$.each(bodyfontcontrol.yourseobookfontslist[index].variants, function(val, text) {
		...
		$(this).parent().parent().find('.google-fonts-category').val(bodyfontcontrol.yourseobookfontslist[index].category);

		yourseobookGetAllSelects($(this).parent().parent());
	});
	...
	function yourseobookGetAllSelects($element) {
		var selectedFont = {
	...
	$('.customize-control-tinymce-editor').each(function(){
		...
		var tinyMCEToolbar1String = _wpCustomizeSettings.controls[$(this).attr('id')].yourseobooktinymcetoolbar1;
		var tinyMCEToolbar2String = _wpCustomizeSettings.controls[$(this).attr('id')].yourseobooktinymcetoolbar2;
		var tinyMCEMediaButtons = _wpCustomizeSettings.controls[$(this).attr('id')].yourseobookmediabuttons;
